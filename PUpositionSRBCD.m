...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [pos,num_iter] = PUpositionSRBCD(X,Y,R,noiseSTD,initialy)
% SR-GD method in:
% Robust Target Localization Based on Squared Range Iterative Reweighted Least Squares
% Alireza Zaeemzadeh, Mohsen Joneidi, Behzad Shahrasbi and Nazanin Rahnavard 
% 14th IEEE International Conference on Mobile Ad hoc and Sensor Systems (MASS) 2017
...
% pos : Target Position
    ...
    yprev = initialy/normalize;     % estimated y in k-2 iteration
    yprev(3) = norm(yprev(1:2))^2;    % estimated y should satisfy the constraint
    yhat = yprev;           % estimated y in k-1 iteration
    wk = 0;                 % extrapolation factor in k iteration
    delta = 1;
    %% 
    maxIter = 1000;
    alpha = 1.34*sqrt(3)*noiseSTD;         % used for shrinkage (robustness to noise section of the paper)
...
%     figure(3);hold on;loglog(yintime,'--k','LineWidth',4,'DisplayName',' SR-GD'); drawnow
%     figure(4);hold on;loglog(time,yintime,'--k','LineWidth',4,'DisplayName',' SR-GD'); drawnow
    pos = yhat(1:2);
    pos = pos*normalize;
    num_iter = i;
end
